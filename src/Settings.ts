...
export class Settings {
  ...
  public static load(): void {
    ...
    try {
      ...
      Debug.write(
        `Could not read settings.json, using defaults. Error: ${(e as Error).stack}`,
      );
    }
    ...
    if (saved.view)
      this.view = { ...this.view, ...(saved.view as typeof this.view) };
    if (saved.timings)
      this.timings = {
        ...this.timings,
        ...(saved.timings as typeof this.timings),
      };
    if (saved.update)
      this.update = { ...this.update, ...(saved.update as typeof this.update) };
  }
}
