...
pub fn build_status(
    ...
    if settings.view.advanced.enabled {
        ...
        (
            crop(&parts.join(" "), MAX_STATUS_LENGTH),
            settings.view.emoji.clone(),
        )
    }
}
...
fn render_template(template: &str, playback: &Playback, line: &LyricsLine) -> String {
    ...
    let tokens = [
        (
            "{lyrics_upper_letters_only}",
            letters_only(&line.text.to_uppercase()),
        ),
        (
            "{lyrics_lower_letters_only}",
            letters_only(&line.text.to_lowercase()),
        ),
